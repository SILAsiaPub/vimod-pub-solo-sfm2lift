...
<!--
    ...
    # Created:      	2014- -
    # Updated		2017-02-27 Added for case of project.tasks occuring in root of project path.
...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:f="myfunctions">
      ...
      <xsl:variable name="classicprojecttasks" select="concat($projectpath,'\setup\project.tasks')"/>
      <xsl:variable name="soloprojecttasks" select="concat($projectpath,'\project.tasks')"/>
      <xsl:variable name="classictask" select="f:file2lines($classicprojecttasks)"/>
      <xsl:variable name="solotask" select="f:file2lines($soloprojecttasks)"/>
      <xsl:variable name="projecttype">
            <xsl:choose>
                  <xsl:when test="unparsed-text-available(f:file2uri($classicprojecttasks))">
                        <xsl:text>classic</xsl:text>
                  </xsl:when>
                  <xsl:when test="unparsed-text-available(f:file2uri($soloprojecttasks))">
                        <xsl:text>solo</xsl:text>
                  </xsl:when>
                  <xsl:otherwise>No projects.tasks found</xsl:otherwise>
            </xsl:choose>
      </xsl:variable>
      <xsl:variable name="cd">
            <xsl:choose>
                  <xsl:when test="$projecttype = 'classic'">
                        <xsl:value-of select="substring-before($projectpath,'\data\')"/>
                  </xsl:when>
                  <xsl:otherwise>
      ...
      <xsl:template match="/">
            <xsl:element name="xsl:stylesheet">
                  ...
                  <xsl:comment select="unparsed-text-available(f:file2uri(concat($projectpath,'\project.tasks')))"/>
                  ...
                  <xsl:choose>
                        <xsl:when test="$projecttype = 'classic'">
                              <xsl:for-each select="$classictask">
                                    <!-- create xslt param and variables for classic-->
                                    <xsl:call-template name="parseline">
                                          <xsl:with-param name="line" select="."/>
                                    </xsl:call-template>
                              </xsl:for-each>
                        </xsl:when>
                        <xsl:when test="$projecttype = 'solo'">
                              <xsl:for-each select="$solotask">
                                    <!-- create xslt param and variables for solo -->
                                    <xsl:call-template name="parseline">
                                          <xsl:with-param name="line" select="."/>
                                    </xsl:call-template>
                              </xsl:for-each>
                        ...
                        <xsl:otherwise>
                              <xsl:comment select="$projecttype"/>
                        </xsl:otherwise>
                  </xsl:choose>
            </xsl:element>
      </xsl:template>
